...
import { createSun } from "./power.js";
...
{
let sun = power[i];
if(sun.collidesWith(player)){
if(player.energy<MAX_ENERGY-30)
player.energy+=30;
else
...
const createTower = (x, floorCount) => {
  ...
  for (let i = 0; i < floorCount; i++) {
    ...
    if (isHoleOnPreviousLayer || random() < 0.8) {
      ...
      if (random() < 0.4) {
        let sun = createSun();
        sun.x = floorLeft + random(floorWidth - sun.width);
        sun.y = floorTop - sun.height - 20;
        power.push(sun);
      }
